...
<div class="training-home">
	...
	<div class="training-home-layout">
		<TrainingSessionComposer
			...
			isLoadingLists={wordLists.isPending}
			isListError={wordLists.isError}
			{totalVocabulary}
		/>
			...
			{:else if stats.data}
				...
				<div class="training-mastery-list">
					{#each Object.entries(stats.data.masteryDistribution) as [level, count]}
						<div class="training-mastery-row">
							<span>{formatTrainingMode(level)}</span>
							<div><i style:width={`${totalVocabulary === 0 ? 0 : (count / totalVocabulary) * 100}%`}></i></div>
							<strong>{count}</strong>
						</div>
					{/each}
				</div>
