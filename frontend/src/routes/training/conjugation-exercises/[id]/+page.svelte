...
{:else if currentItem}<main class="exercise-runner"><header class="exercise-progress-header"><a href="/training/exercises/conjugation">Exit exercise</a><span>Board {currentIndex + 1} of {totalItems}</span></header><div class="exercise-progress-track" aria-hidden="true"><i style:width={`${progress}%`}></i></div><ConjugationMatchBoard item={currentItem} isSubmitting={answerItem.isPending} onsubmit={submit} />{#if answerItem.error}<p class="form-error-msg">{answerItem.error.message}</p>{/if}{#if currentItem.result !== undefined}<div class="exercise-runner-actions"><Button onclick={next} disabled={completeSession.isPending}>{currentIndex === totalItems - 1 ? 'Finish session' : 'Next board'}</Button></div>{/if}</main>
