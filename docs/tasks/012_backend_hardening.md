...
- [x] 12.4 `[B]` Backup setup: Ofelia cron container in `docker-compose.yml`; hourly `pg_dump --format=custom` to named volume; `just restore F=<file>` recipe
- [x] 12.5 `[B]` `./gradlew test` timing baseline: must complete in under 2 minutes on a warm Testcontainers image
- [x] 12.6 `[I]` GitHub Actions `release.yml`: on `v*` tags — build Docker image, push to GHCR, create release with git-cliff changelog
