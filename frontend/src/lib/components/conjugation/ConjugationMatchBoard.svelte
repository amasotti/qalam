	...
	<div class="conj-exercise-columns">
		<div class="conj-exercise-column" role="group" aria-label="Arabic forms">
			...
			{#each item.forms as form (form.formId)}
				<button type="button" class="conj-exercise-form" class:selected={selectedFormId === form.formId} class:paired={mappings[form.formId] !== undefined} class:correct={isAnswered && correctByForm.get(form.formId) === (item.submittedMappings ?? []).find((mapping) => mapping.formId === form.formId)?.labelId} class:incorrect={isAnswered && correctByForm.get(form.formId) !== (item.submittedMappings ?? []).find((mapping) => mapping.formId === form.formId)?.labelId} aria-pressed={selectedFormId === form.formId} disabled={isAnswered || isSubmitting} onclick={() => selectForm(form.formId)}>
					<ConjugatedForm arabic={form.arabic} segments={form.segments} />
					{#if mappings[form.formId] && !isAnswered}<small>Paired — select to change</small>{/if}
				</button>
			{/each}
		</div>
		<div class="conj-exercise-column" role="group" aria-label="Morphology labels">
			...
			{#each item.labels as label (label.labelId)}
				<button type="button" class="conj-exercise-label" class:selected={Object.values(mappings).includes(label.labelId)} disabled={!selectedFormId || isAnswered || isSubmitting} onclick={() => selectLabel(label.labelId)}>{label.label}</button>
			{/each}
		</div>
	</div>
	...
	{#if !isAnswered && Object.keys(mappings).length > 0}
		<div class="conj-exercise-pairs" aria-label="Current pairings">
			{#each item.forms.filter((form) => mappings[form.formId]) as form (form.formId)}
				<button type="button" onclick={() => clearPair(form.formId)}>{form.arabic} → {item.labels.find((label) => label.labelId === mappings[form.formId])?.label} <span aria-hidden="true">×</span></button>
			{/each}
		</div>
	{/if}

	{#if isAnswered}
		<p class:correct={item.result === 'CORRECT'} class:incorrect={item.result === 'INCORRECT'} class="conj-exercise-feedback">{item.result === 'CORRECT' ? 'All four matches are correct.' : 'Review the highlighted forms, then continue.'}</p>
	{:else}
		<div class="conj-exercise-actions"><Button onclick={submit} disabled={!allMapped || isSubmitting}>{isSubmitting ? 'Checking…' : 'Check answers'}</Button></div>
	{/if}
</section>
